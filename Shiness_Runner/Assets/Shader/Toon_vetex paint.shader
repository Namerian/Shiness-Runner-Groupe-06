...
Shader "Custom/ToonVertexpaint" {
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma target 3.0
            uniform sampler2D _DiffuseOld; uniform float4 _DiffuseOld_ST;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float4 _DiffuseOld_var = tex2D(_DiffuseOld,TRANSFORM_TEX(i.uv0, _DiffuseOld));
                clip(_DiffuseOld_var.a - 0.5);
        ...
        Pass {
            ...
            uniform sampler2D _GroundDiff; uniform float4 _GroundDiff_ST;
            uniform sampler2D _SandDiff; uniform float4 _SandDiff_ST;
...
////// Emissive:
                float4 _SandDiff_var = tex2D(_SandDiff,TRANSFORM_TEX(i.uv0, _SandDiff));
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float4 _GroundDiff_var = tex2D(_GroundDiff,TRANSFORM_TEX(i.uv0, _GroundDiff));
                float3 emissive = (UNITY_LIGHTMODEL_AMBIENT.rgb*(_Color.rgb*lerp(lerp(_SandDiff_var.rgb,_Diffuse_var.rgb,i.vertexColor.r),_GroundDiff_var.rgb,i.vertexColor.g)));
                float3 node_7379 = (_DiffuseOld_var.rgb*_DiffuseColor.rgb);
